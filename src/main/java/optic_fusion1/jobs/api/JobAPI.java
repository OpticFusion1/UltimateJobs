...
import optic_fusion1.jobs.job.JobID;
...
public class JobAPI {
    ...
    public JobAPI(UltimateJobs plugin) {
        ...
        this.plugin = plugin;
        up = plugin.getPluginManager();
        bossBarHandler = plugin.getBossBarHandler();
    }
        ...
        if (files != null) {
            for (int i = 0; i < files.length; i++) {
                ...
                if (file.isFile()) {
                    ...
                    ArrayList<JobType> realactions = new ArrayList<JobType>();

                    actions.forEach((ac) -> {

                        try {
                            realactions.add(JobType.valueOf(ac));
                            if (plugin.getLocalFileManager().getUtilsConfig()
                                    .getBoolean("Plugin.DebugMessagesOnStart.Info")) {
                                Bukkit.getConsoleSender().sendMessage(PluginColor.JOB_RELATED_INFO.getPrefix()
                                        + "Loading Job, adding Action " + JobType.valueOf(ac).toString());
                            }
                        } catch (IllegalArgumentException ex) {
                            if (plugin.getLocalFileManager().getUtilsConfig()
                                    .getBoolean("Plugin.DebugMessagesOnStart.Error")) {
                    ...
                    HashMap<JobType, HashMap<String, JobID>> ids = new HashMap<JobType, HashMap<String, JobID>>();
                    ...
                    if (fails.size() == 0) {
                        Job job = new Job(plugin, id, realactions, icon, slot, price, permission,
    ...
    public boolean canWorkThereByPlayer(String UUID, Job j, String st) {
        ...
        if (Bukkit.getPlayer(UUID) == null) {
            loc = plugin.getLocationAPI().getLocation("LastLoc." + UUID);
        } else {
    ...
    public ArrayList<String> getJobsWithAction(String UUID, JobType ac) {
        if (!plugin.getPlayerAPI().getCurrentJobs(UUID).isEmpty()
            ...
            for (String j : plugin.getPlayerAPI().getCurrentJobs(UUID)) {
                ...
                for (JobType actn : jb.getActionList()) {
                    if (actn.equals(ac)) {
    ...
    public boolean canReward(Job j, String id, JobType ac) {
